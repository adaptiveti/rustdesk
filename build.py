...
hbb_name = 'SuporteAdaptive' + ('.exe' if windows else '')
...
def generate_control_file(version):
    ...
    content = """Package: suporteadaptive
Version: %s
Architecture: %s
Maintainer: adaptive <info@adaptive.com.br>
Homepage: https://adaptive.com.br
...
def build_flutter_deb(version, features):
    ...
    system2('mkdir -p tmpdeb/usr/lib/suporteadaptive')
    system2('mkdir -p tmpdeb/etc/suporteadaptive/')
    system2('mkdir -p tmpdeb/etc/pam.d/')
    system2('mkdir -p tmpdeb/usr/share/suporteadaptive/files/systemd/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
    system2('mkdir -p tmpdeb/usr/share/applications/')
    system2('mkdir -p tmpdeb/usr/share/polkit-1/actions')
    system2('rm tmpdeb/usr/bin/SuporteAdaptive || true')
    system2(
        f'cp -r {flutter_build_dir}/* tmpdeb/usr/lib/suporteadaptive/')
    system2(
        'cp ../res/rustdesk.service tmpdeb/usr/share/suporteadaptive/files/systemd/suporteadaptive.service')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/suporteadaptive.png')
    system2(
        'cp ../res/scalable.svg tmpdeb/usr/share/icons/hicolor/scalable/apps/suporteadaptive.svg')
    system2(
        'cp ../res/rustdesk.desktop tmpdeb/usr/share/applications/suporteadaptive.desktop')
    system2(
        'cp ../res/rustdesk-link.desktop tmpdeb/usr/share/applications/suporteadaptive-link.desktop')
    system2(
        'cp ../res/com.rustdesk.RustDesk.policy tmpdeb/usr/share/polkit-1/actions/')
    system2(
        'cp ../res/startwm.sh tmpdeb/etc/suporteadaptive/')
    system2(
        'cp ../res/xorg.conf tmpdeb/etc/suporteadaptive/')
    system2(
...
def build_deb_from_folder(version, binary_folder):
    ...
    system2('rm tmpdeb/usr/bin/SuporteAdaptive || true')
    system2(
        f'cp -r ../{binary_folder}/* tmpdeb/usr/lib/suporteadaptive/')
    system2(
        'cp ../res/rustdesk.service tmpdeb/usr/share/suporteadaptive/files/systemd/suporteadaptive.service')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/suporteadaptive.png')
...
def build_flutter_windows(version, features):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/SuporteAdaptive.exe')
    os.chdir('../..')
...
def main():
    ...
    '''.format(pa, version))
                ...
                system2(
                    'cp res/rustdesk-link.desktop tmpdeb/usr/share/applications/suporteadaptive-link.desktop')
                os.system('mkdir -p tmpdeb/etc/suporteadaptive/')
                os.system('cp -a res/startwm.sh tmpdeb/etc/suporteadaptive/')
                os.system('mkdir -p tmpdeb/etc/X11/suporteadaptive/')
                os.system('cp res/xorg.conf tmpdeb/etc/X11/suporteadaptive/')
                os.system('cp -a DEBIAN/* tmpdeb/DEBIAN/')
                os.system('mkdir -p tmpdeb/etc/pam.d/')
                os.system('cp pam.d/rustdesk.debian tmpdeb/etc/pam.d/suporteadaptive')
                system2('strip tmpdeb/usr/bin/suporteadaptive')
                system2('mkdir -p tmpdeb/usr/lib/suporteadaptive')
                system2('mv tmpdeb/usr/bin/SuporteAdaptive tmpdeb/usr/lib/suporteadaptive/')
                system2('cp libsciter-gtk.so tmpdeb/usr/lib/suporteadaptive/')
                md5_file('usr/share/suporteadaptive/files/systemd/suporteadaptive.service')
                md5_file('etc/suporteadaptive/startwm.sh')
                md5_file('etc/X11/suporteadaptive/xorg.conf')
                md5_file('etc/pam.d/suporteadaptive')
                md5_file('usr/lib/suporteadaptive/libsciter-gtk.so')
